    ...
    <View className="flex-1 bg-white">
      ...
      >
        ...
        <View className="relative">
          <Image
            source={{ uri: getImageUrl(location.image || location.img) }}
        ...
        <View className="px-5 py-6 -mt-6 bg-white rounded-t-3xl">
          ...
          <View className="flex-row flex-wrap gap-3 mb-6">
            {["Toilet", "Kantin", "Parkir Luas", "Mushola", "Sewa Alat"].map(
              (f, i) => (
                <View
                  key={i}
                ...
                >
                  ...
                  <Text className="ml-2 text-gray-600 text-sm">{f}</Text>
                </View>
              )
            )}
          </View>
          ...
          {/* 1. Map Card */}
          <LocationMapCard address={location.address} city={location.city} />
      ...
      <View className="absolute bottom-0 w-full bg-white p-5 border-t border-gray-100 shadow-2xl pb-8">
        <View className="flex-row items-center justify-between">
          <View>
            <Text className="text-gray-500 text-xs">Harga Tiket</Text>
            <Text className="text-2xl font-bold text-blue-600 font-[Outfit_700Bold]">
              Rp {Number(location.price || 50000).toLocaleString("id-ID")}
            </Text>
          </View>
          <TouchableOpacity
            onPress={() =>
