...
async function check() {
  ...
  try {
    ...
    try {
      const [locs] = await connection.execute(
        "SELECT COUNT(*) as count FROM locations",
      );
      console.log(`📊 Locations count in DB: ${locs[0].count}`);
    } catch (e) {
      console.log("❌ Error querying locations table:", e.message);
    }
    ...
    try {
      ...
      console.log("❌ Error querying products table:", e.message);
    }
  ...
  try {
    console.log(
      "Testing API connection to http://127.0.0.1:3000/locations ...",
    );
    const res = await fetch("http://127.0.0.1:3000/locations");
    if (res.ok) {
      const data = await res.json();
      console.log(
        `✅ API /locations reachable. Returned ${Array.isArray(data) ? data.length : "not-array"} items.`,
      );
    } else {
